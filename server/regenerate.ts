...
import { updateGeneratedAt } from '../shared/metadata.ts'
import { generateHTMLTemplate } from '../shared/html-template.ts'
import type { RouteNode, ClientComponentMap } from '../shared/types.ts'
...
export interface RegenerateOptions {
  /** 路由路径节点数组(用于 Layout 嵌套) */
  routePathNodes: RouteNode[]
  /** Client Component 映射表 */
 ...
 * @param routePathNodes - 路由路径节点数组(用于 Layout 嵌套)
 * @param clientComponentMap - Client Component 映射表
 * @param htmlPath - HTML 文件路径
 * @param flightPath - Flight 文件路径
 */
async function regeneratePage(
  ...
  try {
    ...
    console.error(`  ❌ ISR: 重新生成失败 ${routePath}`, message)
    throw error
  }
}
 ...
 * @param options - 重新生成选项
 */
export function regenerateInBackground(
  routePath: string,
  options: RegenerateOptions
): void {
  // 启动后台任务，不等待完成
