...
import { CHUNK_TYPES, FLIGHT_MARKERS } from './flight-types.ts'
import type { ModuleInfo, ModuleMap, FlightElementArray } from './types.ts'
...
export class FlightDecoder {
   ...
   *
   * @param flightData - RSC Flight Protocol 字符串数据
   * @returns 解析后的 React 元素树
   *
   ...
   */
  decode(flightData: string): React.ReactElement {
    if (!flightData?.trim()) {
    ...
    const modules = new Map<string, ChunkData>()
   ...
   *
   * @param line - 单行数据，格式: {type}{id}:{data}
   * @param modules - 模块存储 Map
   * @param chunks - 数据块存储 Map
   *
   * 例如:
  ...
  private parseModuleChunk(
    ...
    try {
      modules.set(`M${id}`, {
        type: 'module',
        info: JSON.parse(dataStr) as ModuleInfo
      })
    } catch (error) {
   ...
   *
   * @param id - chunk ID
   * @param dataStr - Symbol 字符串
   * @param chunks - 数据块存储 Map
   */
  private parseSymbolChunk(
    id: string,
    dataStr: string,
    chunks: Map<string, ChunkData>
  ): void {
   ...
   *
   * @param id - chunk ID
   * @param dataStr - JSON 字符串，包含错误信息
   * @param chunks - 数据块存储 Map
   */
  private parseErrorChunk(
    id: string,
    dataStr: string,
    chunks: Map<string, ChunkData>
  ): void {
   ...
   * 根据 chunk ID 解析对应的数据
   *
   * @param chunkId - chunk ID (如 'J0', 'M1', 'S1' 等)
   * @param modules - 模块存储 Map
   * @param chunks - 数据块存储 Map
  ...
  private resolveChunk(
    ...
    if (module && module.info) return this.loadClientComponent(module.info)
  ...
  private resolveValue(
    ...
    if (Array.isArray(value)) {
      ...
      return value[0] === FLIGHT_MARKERS.MODULE_REF
        ? this.resolveElement(value as FlightElementArray, modules, chunks)
   ...
   * 解析 React 元素
   *
   * @param element - React 元素数组 ['$', type, key, props]
   * @param modules - 模块存储 Map
   * @param chunks - 数据块存储 Map
   * @returns React 元素
   *
  ...
  private resolveElement(
    [, type, key, props]: FlightElementArray,
    modules: Map<string, ChunkData>,
    chunks: Map<string, ChunkData>
   ...
   */
  loadClientComponent({ id, name }: ModuleInfo): React.ComponentType<any> {
    return (props: any) => {
